...
License: GPLv2
*/
update_option( 'pp_envato_personal_token', 'prowebber' );
...
function avante_add_admin() 
{
	...
	if (is_admin() && current_user_can('manage_options') && isset($_REQUEST['_wpnonce']) && wp_verify_nonce($_REQUEST['_wpnonce'], 'avante_save_theme_setting') && isset($_REQUEST['action']) && 'save' == $_REQUEST['action']) {
		...
		if(isset($_REQUEST['pp_envato_personal_token']) && !empty($_REQUEST['pp_envato_personal_token']) && $_REQUEST['pp_envato_personal_token'] != '[ThemeGoods Activation]')
		{
			...
			{
				$is_verified_envato_purchase_code = false;
				delete_option("pp_verified_envato_avante", true);
			}
		...
		<?php 
			foreach ($avante_options as $value) {
				...
				if($value['type'] == 'section')
				{
					...
					echo '<a id="setting-panel-'.strtolower($value['name']).'-a" href="#setting-panel-'.strtolower($value['name']).'" class="nav-tab '.esc_attr($active).'"><span class="dashicons  '.$value['icon'].'"></span>'.str_replace('-', ' ', $value['name']).'</a>';
				}
			}
		?>
		</h2>
